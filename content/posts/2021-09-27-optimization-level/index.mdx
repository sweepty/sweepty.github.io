...
### assert가 뭔가요?
assert는 디버깅을 할 때 오류가 생기면 안되는 곳에 작성하여 디버깅을 쉽게 하기 위한 함수.
만약 `assert`에 조건을 만족하지 못하는 값이 들어오면 메세지를 출력한 후 프로그램이 중단되고 디버그 가능한 상태로 전환된다.
...
타입 추론이나 오버플로우 검사도 하지 않기 때문에 일반적으로 사용할 수 없는 옵션입니다.
